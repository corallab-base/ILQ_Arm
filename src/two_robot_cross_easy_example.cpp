...
#include <ilqgames/examples/two_robot_cross_easy_example.h>
...
// Robot Base
const Eigen::Vector3d rob1_tran = Eigen::Vector3d(-0.1, -0.8, 0);
const Eigen::Quaterniond rob1_quat(0.70710678, 0.0, 0.0, 0.70710678);

const Eigen::Vector3d rob2_tran = Eigen::Vector3d(0.1, 0.8, 0);
...
// box location & size
const double box_xd = 0.15;
const double box_yd = 0.60;
const double box_zd = 0.70;

const double box_x = 0.6;
const double box_y = 0.0;
const double box_z = box_zd/2;

// Moving axis & dist
size_t move_axis = 1;
double move_dist = 0.7;
...
static const Eigen::VectorXd rob1_init_q = []{
    Eigen::VectorXd v(6);
    v << 0, -M_PI/4, M_PI/4, -M_PI/2, -M_PI/2, 0;
    return v;
}();
...
static const Eigen::VectorXd rob1_goal_q = []{
    Eigen::VectorXd v(6);
    v << -M_PI/4, -M_PI/4, M_PI/4, -M_PI/2, -M_PI/2, 0;
    return v;
}();
...
static constexpr double kEnvCollisionConstWeight = 50.0;
...
static const Eigen::VectorXd kGoalCostWeight = []{
    Eigen::VectorXd v(6);
    v << 70.0, 70.0, 70.0, 70.0, 70.0, 70.0;
    return v;
}();
...
void TwoRobotCrossEasyExample::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotCrossEasyExample::ConstructInitialState() {
...
void TwoRobotCrossEasyExample::ConstructPlayerCosts() {
    ...
    if (is_floor_on || is_box_on) {
        ...
        if (is_box_on && !is_move_on) {
            ...
            box.size = Eigen::Vector3d(box_xd, box_yd, box_zd);
            box.speed = move_dist / time::kTimeHorizon; // m/s

            moving_obs.push_back(box);
...
inline std::vector<double> TwoRobotCrossEasyExample::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotCrossEasyExample::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotCrossEasyExample::get_sdf() const {
    return env;
}

std::vector<MovingObs>& TwoRobotCrossEasyExample::get_moving_obs() const {
    return moving_obs;
}

...
#include <ilqgames/examples/two_robot_complex_env_demo4.h>
...
// Robot Base
const Eigen::Vector3d rob1_tran = Eigen::Vector3d(0.0, -0.0, 0);
const Eigen::Quaterniond rob1_quat(1.0, 0.0, 0.0, 0.0);

const Eigen::Vector3d rob2_tran = Eigen::Vector3d(0.6, 0.0, 0);
const Eigen::Quaterniond rob2_quat(0.0, 0.0, 0.0, 1.0);
...
static const Eigen::VectorXd rob1_init_q = []{
    Eigen::VectorXd v(6);
    v << M_PI/4, -M_PI/2, M_PI/2, M_PI/2, M_PI/2, 0.0;
    return v;
}();
...
static const Eigen::VectorXd rob1_goal_q = []{
    Eigen::VectorXd v(6);
    v << M_PI/2,-M_PI/3, M_PI/3, 0, M_PI/2, 0;
    return v;
}();

static const Eigen::VectorXd rob2_goal_q = []{
    Eigen::VectorXd v(6);
    v << 0,-M_PI/3, M_PI/3, 0, M_PI/2, 0;
    return v;
}();
...
void TwoRobotComplexEnvDemo4::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotComplexEnvDemo4::ConstructInitialState() {
...
void TwoRobotComplexEnvDemo4::ConstructPlayerCosts() {
...
inline std::vector<double> TwoRobotComplexEnvDemo4::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotComplexEnvDemo4::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotComplexEnvDemo4::get_sdf() const {
    return env;
}

std::vector<MovingObs>& TwoRobotComplexEnvDemo4::get_moving_obs() const {
    return moving_obs;
}

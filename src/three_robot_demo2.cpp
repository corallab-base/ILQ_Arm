...
#include <ilqgames/examples/three_robot_demo2.h>
...
// Robot Base
const Eigen::Vector3d rob1_tran = Eigen::Vector3d(0.3, -0.5, 0);
const Eigen::Quaterniond rob1_quat(0.70710678, 0.0, 0.0, 0.70710678);

const Eigen::Vector3d rob2_tran = Eigen::Vector3d(-0.3, -0.5, 0);
const Eigen::Quaterniond rob2_quat(0.70710678, 0.0, 0.0, 0.70710678);

const Eigen::Vector3d rob3_tran = Eigen::Vector3d(0.0, 0.5, 0);
const Eigen::Quaterniond rob3_quat(0.70710678, 0.0, 0.0, -0.70710678);
...
static const Eigen::VectorXd rob3_init_q = []{
    Eigen::VectorXd v(6);
    v << -M_PI/2, -M_PI/4, M_PI/4, -M_PI/2, -M_PI/2, 0;
    return v;
}();
...
void ThreeRobotDemo2::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2, p3}));
}

void ThreeRobotDemo2::ConstructInitialState() {
...
void ThreeRobotDemo2::ConstructPlayerCosts() {
...
inline std::vector<double> ThreeRobotDemo2::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> ThreeRobotDemo2::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot(), &p3->get_robot()};
}


std::vector<std::shared_ptr<SignedDistanceField>>& ThreeRobotDemo2::get_sdf() const {
    return env;
}

...
#include <ilqgames/examples/two_robot_demo1.h>
...
static const Eigen::VectorXd rob1_init_q = []{
    Eigen::VectorXd v(6);
    v << 0.66852, -0.451483, 0.501359, 0.724185, 0.656654, -0.863779;
    return v;
}();

static const Eigen::VectorXd rob2_init_q = []{
    Eigen::VectorXd v(6);
    v << -0.103542, -0.903303, 0.928023, 0.174609, -0.993895, 0.413151;
    return v;
}();
...
static const Eigen::VectorXd rob1_goal_q = []{
    Eigen::VectorXd v(6);
    v << -0.995455, -0.807009, 0.876798, -0.248125, -0.940567, 0.191326;
    return v;
}();

static const Eigen::VectorXd rob2_goal_q = []{
    Eigen::VectorXd v(6);
    v << -0.705402, -0.367734, -0.11472, -0.0433363, -0.584591, 0.23638;
    return v;
}();
...
void TwoRobotDemo1::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotDemo1::ConstructInitialState() {
...
void TwoRobotDemo1::ConstructPlayerCosts() {
...
inline std::vector<double> TwoRobotDemo1::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotDemo1::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotDemo1::get_sdf() const {
    return env;
}

std::vector<MovingObs>& TwoRobotDemo1::get_moving_obs() const {
    return moving_obs;
}

...
#include <ilqgames/examples/two_robot_complex_env_demo2.h>
...
static const Eigen::VectorXd rob1_init_q = []{
    Eigen::VectorXd v(6);
    v << 0.0, -M_PI/3, M_PI/3, 0.0, M_PI/2, 0;
    return v;
}();
...
static constexpr double kEnvCollisionConstWeight = 10.0;
...
void TwoRobotComplexEnvDemo2::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotComplexEnvDemo2::ConstructInitialState() {
...
void TwoRobotComplexEnvDemo2::ConstructPlayerCosts() {
...
inline std::vector<double> TwoRobotComplexEnvDemo2::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotComplexEnvDemo2::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotComplexEnvDemo2::get_sdf() const {
    return env;
}

std::vector<MovingObs>& TwoRobotComplexEnvDemo2::get_moving_obs() const {
    return moving_obs;
}

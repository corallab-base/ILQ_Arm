...
#include <ilqgames/examples/two_robot_intersect_easy_example.h>
...
static const Eigen::VectorXd rob1_init_q = []{
    Eigen::VectorXd v(6);
    v <<-0.10949638, -2.17604574,  0.41169264,  0.22345049, -1.51792423, 1.57;
    return v;
}();

static const Eigen::VectorXd rob2_init_q = []{
    Eigen::VectorXd v(6);
    v <<-0.10949638, -2.17604574,  0.41169264,  1.22345049, -1.51792423, 1.57;
    return v;
}();

// goal joints1
static constexpr float goal1_start_window = 1.0;
static const Eigen::VectorXd rob1_goal_q = []{
    Eigen::VectorXd v(6);
    v <<-0.10949638, -2.17604574,  0.41169264,  1.22345049, 2.01792423, 3.06321871;
    return v;
}();
...
// Collision threshold
static constexpr double kCollisionConstWeight = 100.0;
...
static constexpr double envCollision_threshold = 0.05;
...
void TwoRobotIntersectEasyExample::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotIntersectEasyExample::ConstructInitialState() {
...
void TwoRobotIntersectEasyExample::ConstructPlayerCosts() {
    ...
    const auto P1_goal_cost = std::make_shared<FinalTimeCostDouble>(
        std::make_shared<QuadraticRobCost>(kGoalCostWeight, kP1Qvec, rob1_goal_q, true),
    ...
    const auto P2_goal_cost = std::make_shared<FinalTimeCostDouble>(
        std::make_shared<QuadraticRobCost>(kGoalCostWeight, kP2Qvec, rob2_goal_q, true),
    ...
    if (is_floor_on || is_box_on) {
        ...
        if (is_box_on) {
            Eigen::Vector3d center(1.0, 0.0, 0.5);
            Eigen::Vector3d size(0.5, 0.5, 1.0);
            auto sdf_box = std::make_shared<SignedDistanceField>(
                SignedDistanceField::CreateBox(origin, res, dim, center, size)
            );
            env.push_back(sdf_box);
        }
...
inline std::vector<double> TwoRobotIntersectEasyExample::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotIntersectEasyExample::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotIntersectEasyExample::get_sdf() const {
    return env;
}

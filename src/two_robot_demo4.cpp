...
static const Eigen::VectorXd rob1_init_q = []{
    Eigen::VectorXd v(6);
    v << 0.79836, -0.50706, 0.515671, 0.516206, 0.973769, -0.764577;
    return v;
}();

static const Eigen::VectorXd rob2_init_q = []{
    Eigen::VectorXd v(6);
    v << -0.604909, -0.864087, -0.0657353, -0.332615, -0.407466, -0.571562;
    return v;
}();
...
static const Eigen::VectorXd rob1_goal_q = []{
    Eigen::VectorXd v(6);
    v << -0.799081, -0.784453, -0.0828815, 0.773771, -0.998593, -0.0964106;
    return v;
}();

static const Eigen::VectorXd rob2_goal_q = []{
    Eigen::VectorXd v(6);
    v << -0.805148, -0.49231, -0.713615, -0.718633, -0.692182, 0.603551;
    return v;
}();
...
void TwoRobotDemo4::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotDemo4::ConstructInitialState() {
...
void TwoRobotDemo4::ConstructPlayerCosts() {
...
inline std::vector<double> TwoRobotDemo4::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotDemo4::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotDemo4::get_sdf() const {
    return env;
}

std::vector<MovingObs>& TwoRobotDemo4::get_moving_obs() const {
    return moving_obs;
}

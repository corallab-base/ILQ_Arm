...
#include <ilqgames/examples/two_robot_demo3.h>
...
static const Eigen::VectorXd rob1_init_q = []{
    Eigen::VectorXd v(6);
    v << -0.463022, -0.759166, -0.120334, 0.204927, -0.379088, 0.188675;
    return v;
}();

static const Eigen::VectorXd rob2_init_q = []{
    Eigen::VectorXd v(6);
    v << 0.901243, -0.656564, 0.547897, -0.539136, 0.621223, -0.0567054;
    return v;
}();
...
static const Eigen::VectorXd rob1_goal_q = []{
    Eigen::VectorXd v(6);
    v << -0.920045, -0.55168, -0.72392, 0.356199, 0.477007, 0.916461;
    return v;
}();

static const Eigen::VectorXd rob2_goal_q = []{
    Eigen::VectorXd v(6);
    v << -0.97489, -0.951958, 0.584558, -0.919448, -0.274643, -0.44893;
    return v;
}();
...
void TwoRobotDemo3::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotDemo3::ConstructInitialState() {
...
void TwoRobotDemo3::ConstructPlayerCosts() {
...
inline std::vector<double> TwoRobotDemo3::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotDemo3::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotDemo3::get_sdf() const {
    return env;
}

std::vector<MovingObs>& TwoRobotDemo3::get_moving_obs() const {
    return moving_obs;
}

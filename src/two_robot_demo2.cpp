...
#include <ilqgames/examples/two_robot_demo2.h>
...
// Robot Base
const Eigen::Vector3d rob1_tran = Eigen::Vector3d(0.0, -0.4, 0);
const Eigen::Quaterniond rob1_quat(0.70710678, 0.0, 0.0, 0.70710678);

const Eigen::Vector3d rob2_tran = Eigen::Vector3d(0.0, 0.4, 0);
...
static const Eigen::VectorXd rob1_init_q = []{
    Eigen::VectorXd v(6);
    v << 0.71465, -0.287876, -0.135425, 0.546492, -0.919781, -0.978558;
    return v;
}();

static const Eigen::VectorXd rob2_init_q = []{
    Eigen::VectorXd v(6);
    v << 0.691369, -0.974703, 0.713356, -0.237755, 0.835584, 0.90535;
    return v;
}();
...
static const Eigen::VectorXd rob1_goal_q = []{
    Eigen::VectorXd v(6);
    v << -0.759139, -0.638871, -0.786672, 0.789041, 0.509341, -0.465744;
    return v;
}();

static const Eigen::VectorXd rob2_goal_q = []{
    Eigen::VectorXd v(6);
    v << -0.949877, -0.695632, 0.486412, 0.419396, -0.00222855, -0.251848;
    return v;
}();
...
void TwoRobotDemo2::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotDemo2::ConstructInitialState() {
...
void TwoRobotDemo2::ConstructPlayerCosts() {
...
inline std::vector<double> TwoRobotDemo2::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotDemo2::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotDemo2::get_sdf() const {
    return env;
}

std::vector<MovingObs>& TwoRobotDemo2::get_moving_obs() const {
    return moving_obs;
}

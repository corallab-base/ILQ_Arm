...
#include <ilqgames/examples/two_robot_cross_hard_example.h>
...
void TwoRobotCrossHardExample::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotCrossHardExample::ConstructInitialState() {
...
void TwoRobotCrossHardExample::ConstructPlayerCosts() {
...
inline std::vector<double> TwoRobotCrossHardExample::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotCrossHardExample::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotCrossHardExample::get_sdf() const {
    return env;
}

...
#include <ilqgames/examples/two_robot_complex_env_demo1.h>
...
void TwoRobotComplexEnvDemo1::ConstructDynamics() {
    dynamics_.reset(new ConcatenatedDynamicalSystemDouble({p1, p2}));
}

void TwoRobotComplexEnvDemo1::ConstructInitialState() {
...
void TwoRobotComplexEnvDemo1::ConstructPlayerCosts() {
...
inline std::vector<double> TwoRobotComplexEnvDemo1::Qs(const VectorXd& x) const {
    return {
        x(kP1Q1Idx),
        x(kP1Q2Idx),
        x(kP1Q3Idx),
...
std::vector<Robot*> TwoRobotComplexEnvDemo1::get_robots() const {
    return {&p1->get_robot(), &p2->get_robot()};
}

std::vector<std::shared_ptr<SignedDistanceField>>& TwoRobotComplexEnvDemo1::get_sdf() const {
    return env;
}

std::vector<MovingObs>& TwoRobotComplexEnvDemo1::get_moving_obs() const {
    return moving_obs;
}

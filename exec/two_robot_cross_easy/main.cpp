...
DEFINE_string(experiment_name, "two_robot_cross_easy", "Name for the experiment.");
...
int main(int argc, char** argv) {
  ...
  auto problem = std::make_shared<ilqgames::TwoRobotCrossEasyExample>();
  problem->Initialize();
  ...
  std::vector<ilqgames::MovingObs> moving_obs = problem->get_moving_obs();
  ilqgames::Rob3DRenderer rob_3d_render(sliders, {problem}, robots, env, moving_obs);

...
int main(int argc, char** argv) {
    ...
    // read testcases
    // std::string test_case_name = std::to_string(num_cases) + "_" + std::to_string(num_robots) + "rob_random_pos_list" 
    //                             + (is_floor_on ? "_Wfloor" : "") + (is_box_on ? "_Wbox" : "") + (is_move_on ? "_Wmove" : "")
    //                             + "_" + std::to_string(radius);

    std::string test_case_name = std::to_string(num_cases) + "_" + std::to_string(num_robots) + "rob_custom" 
                                + (is_floor_on ? "_Wfloor" : "") + (is_box_on ? "_Wbox" : "") + (is_move_on ? "_Wmove" : "");

    // std::string test_case_name = std::to_string(num_cases) + "_" + std::to_string(num_robots) + "rob_ablation" 
    //                             + (is_floor_on ? "_Wfloor" : "") + (is_box_on ? "_Wbox" : "") + (is_move_on ? "_Wmove" : "");

    // std::string test_case_name = std::to_string(num_cases) + "_" + std::to_string(num_robots) + "rob_test_ablation" + argv[1] 
    //                             + (is_floor_on ? "_Wfloor" : "") + (is_box_on ? "_Wbox" : "") + (is_move_on ? "_Wmove" : "");
